...
private fun parseEpisodeNumberFromText(text: String): Int? {
    ...
    rx1.find(t)?.let { return it.groupValues[2].toIntOrNull() }

    val rx2 = "(episodio|ep\\.?|episode|capitolo|parte)\\s*(\\d+)".toRegex()
    rx2.find(t)?.let { return it.groupValues[2].toIntOrNull() }

    val rx3 = "\\b(\\d{1,3})\\b".toRegex()
    rx3.find(t)?.let { return it.groupValues[1].toIntOrNull() }
...
class OnlineSerieTvProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // Prima passata: quante stagioni vede il sito?
        ...
        rows.forEach { row ->
            ...
            if (tmdbSeasonsInfo.isNotEmpty()) {
                ...
                if (siteMaxSeason == 1 && tmdbSeasonsInfo.size > 1) {
                    // CASO 1: sito vede 1 stagione, TMDB ne vede 2+
                    ...
                    // CASO 2: sito ha più stagioni → sito è base
